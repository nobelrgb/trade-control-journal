...
function AppInner() {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-500 text-sm">{t.common.connecting}</p>
        </div>
      </div>
    )
  }
  ...
  if (tradesLoading) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-center">
          <Loader2 size={32} className="text-amber-400 animate-spin mx-auto mb-3" />
          <p className="text-zinc-500 text-sm">{t.common.loadingTrades}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <main className={`${isRtl ? 'lg:pr-56' : 'lg:pl-56'} pb-20 lg:pb-0`}>
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-6">
          {/* Top Bar */}
          {activeView !== 'settings' && (
            <div className="flex items-center justify-between mb-6">
              <div className="flex gap-2">
                {trades.length === 0 && activeView === 'dashboard' && (
                  <button
              ...
              <div className="flex gap-2">
                ...
                <label className="flex items-center gap-1.5 px-3 py-1.5 border border-[#2a2a2a] rounded-lg text-zinc-400 hover:text-white hover:border-[#3a3a3a] transition-all text-xs cursor-pointer">
                  <Upload size={13} />
                  {importLoading ? t.common.importing : t.common.importCsv}
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-24 lg:bottom-6 ${isRtl ? 'left-4' : 'right-4'} z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-xl text-sm font-medium ${
          toast.type === 'success'
